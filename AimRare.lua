...
local TweenService = game:GetService("TweenService") -- Für UI Animationen
...
local Settings = {
    ...
    NameESP = false,
    HealthESP = false,
    TeamCheck = false, -- Global Team Check
    ESPColor = SafeColor(255, 65, 65), -- Helles Rot
    ...
    -- Aimbot Checks (Bypasses/Legit)
    WallCheck = false, -- Zielt nicht durch Wände
    AliveCheck = true, -- Nur lebende Ziele (Standard an)
}
...
local Theme = {
    ...
    Accent = SafeColor(255, 65, 65), -- Rot
    Success = SafeColor(100, 255, 120)
}

local MainFrame = Instance.new("Frame")
MainFrame.Name = "MainFrame"
MainFrame.Size = UDim2.new(0, 500, 0, 350) -- Etwas größer für mehr Optionen
...
-- Fix für rechte Ecken der Sidebar
...
CreditTitle.Position = UDim2.new(0, 0, 0.3, 0)
...
CreditTitle.Parent = TabsFrames.Credits

local CreditSub = Instance.new("TextLabel")
CreditSub.Size = UDim2.new(1, 0, 0, 30)
CreditSub.Position = UDim2.new(0, 0, 0.5, 0)
CreditSub.BackgroundTransparency = 1
CreditSub.Text = "Thanks For using it ❤️"
CreditSub.TextColor3 = Theme.Accent
CreditSub.Font = Enum.Font.GothamBold
CreditSub.TextSize = 18
CreditSub.Parent = TabsFrames.Credits
...
local function CreateToggle(parent, text, default, callback)
    ...
    btn.BackgroundColor3 = default and Theme.Accent or SafeColor(60, 60, 60)
    btn.Text = ""
    local btnC = Instance.new("UICorner", btn); btnC.CornerRadius = UDim.new(1, 0)
    
    local circle = Instance.new("Frame", btn)
    circle.Size = UDim2.new(0, 16, 0, 16)
    circle.Position = default and UDim2.new(1, -18, 0.5, -8) or UDim2.new(0, 2, 0.5, -8)
    circle.BackgroundColor3 = Theme.Text
    local circC = Instance.new("UICorner", circle); circC.CornerRadius = UDim.new(1, 0)
...
local function CreateSlider(parent, text, valueKey, min, max, displayFormat)
    ...
    label.BackgroundTransparency = 1
    label.Text = text .. ": " .. string.format(displayFormat, Settings[valueKey])
    label.TextColor3 = Theme.Text
    label.Font = Enum.Font.GothamSemibold
    label.TextSize = 14
    label.TextXAlignment = Enum.TextXAlignment.Left
    
    local sliderBg = Instance.new("TextButton", frame) -- Button for click detection
    sliderBg.Size = UDim2.new(1, -30, 0, 6)
    sliderBg.Position = UDim2.new(0, 15, 0, 40)
    sliderBg.BackgroundColor3 = SafeColor(60, 60, 60)
    sliderBg.Text = ""
    sliderBg.AutoButtonColor = false
    local sC = Instance.new("UICorner", sliderBg); sC.CornerRadius = UDim.new(1, 0)
    
    local fill = Instance.new("Frame", sliderBg)
    local percent = (Settings[valueKey] - min) / (max - min)
...
CreateSection(TabsFrames.Aimbot, "CHECKS & BYPASSES")
CreateToggle(TabsFrames.Aimbot, "Wall Check (Visible)", Settings.WallCheck, function() Settings.WallCheck = not Settings.WallCheck; return Settings.WallCheck end)
CreateToggle(TabsFrames.Aimbot, "Team Check (Aimbot)", Settings.TeamCheck, function() Settings.TeamCheck = not Settings.TeamCheck; return Settings.TeamCheck end)
...
local function IsVisible(target)
    ...
    -- Wenn wir nichts treffen, ist der Weg frei. Wenn wir etwas treffen, ist eine Wand dazwischen.
    if result then
        return false -- Wand getroffen
    end
    return true -- Weg frei
end
...
RunService.RenderStepped:Connect(function()
    ...
    for _, player in pairs(Players:GetPlayers()) do
        if player ~= LocalPlayer and player.Character and player.Character:FindFirstChild("HumanoidRootPart") and player.Character:FindFirstChild("Humanoid") then
            ...
            if onScreen then
                -- FIX: Box Größe angepasst (Größerer Multiplikator)
...
print("AimRare Hub v2.5 (Credits) Loaded")
